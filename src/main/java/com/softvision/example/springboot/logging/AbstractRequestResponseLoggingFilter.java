...
public abstract class AbstractRequestResponseLoggingFilter extends OncePerRequestFilter {
    ...
    private static final int DEFAULT_MAX_RESPONSE_DATA_LENGTH = 1024;
    ...
    private boolean includeResponseData = false;
    ...
    private int maxResponseDataLength = DEFAULT_MAX_RESPONSE_DATA_LENGTH;
    ...
    /**
     * Set whether the response payload (body) should be included in the log message.
     * <p>Should be configured using an {@code <init-param>} for parameter name
     * "includeResponseData" in the filter definition in {@code web.xml}.
     * @since 3.0
     */
    ...
    /**
     * Return whether the response payload (body) should be included in the log message.
     * @since 3.0
     */
    protected boolean isIncludeResponseData() {
    ...
    /**
     * Set the maximum length of the ResponseData body to be included in the log message.
     * Default is 50 characters.
     * @since 3.0
     */
    public void setMaxResponseDataLength(int maxResponseDataLength) {
        Assert.isTrue(maxResponseDataLength >= 0, "'maxResponseDataLength' should be larger than or equal to 0");
        this.maxResponseDataLength = maxResponseDataLength;
    }

    /**
     * Return the maximum length of the ResponseData body to be included in the log message.
     * @since 3.0
     */
    protected int getMaxResponseDataLength() {
        return this.maxResponseDataLength;
    }
     ...
     * @see #createMessage
     */
    private String getAfterResponseMessage(HttpServletResponse response) throws IOException{
    ...
    @Nullable
    protected  String getResponseData(final HttpServletResponse response) throws IOException {
        ...
        if (wrapper != null) {
            byte[] buf = wrapper.getContentAsByteArray();
            if (buf.length > 0) {
                int length = Math.min(buf.length, getMaxResponseDataLength());
                payload = new String(buf, 0, length, wrapper.getCharacterEncoding());
                wrapper.copyBodyToResponse();
            }
        } else {
            logger.info("wrapper null");
        }
        return message.append(payload).toString();
    }

//    @Nullable
//    protected String getMessageResponsePayload(HttpServletResponse response) {
//        logger.info("getMessageResponsePayload");
//        ContentCachingResponseWrapper wrapper =
//                WebUtils.getNativeResponse(response, ContentCachingResponseWrapper.class);
//        logger.info("getMessageResponsePayload wrapper ");
//        if (wrapper != null) {
//            logger.info("getMessageResponsePayload wrapper not null");
//            byte[] buf = wrapper.getContentAsByteArray();
//            logger.info("getMessageResponsePayload byte " + buf);
//            if (buf.length > 0) {
//                int length = Math.min(buf.length, getMaxPayloadLength());
//                try {
//                    return new String(buf, 0, length, wrapper.getCharacterEncoding());
